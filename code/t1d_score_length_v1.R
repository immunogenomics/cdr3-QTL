...
ofile <- paste0("assoc/HIP/t1d_score/length_v1/L", length,"_P", pos ,".lm.v1.txt.gz")

 #phenotype
pdata <- fread(paste0("gunzip -c phenotype.freq3/L",length,"_P",pos,"/single_aa_usage_ratio.txt.gz"), header=T, stringsAsFactors=F)
pdata <- as.data.frame(pdata)
colnames(pdata)[2] <- "pname"
pdata <- pdata[ ,c("Sample", "pname", "rate")]

 #genotype (NEW GENOTYPE)
load("T1D_haplotype_score_v1.RData") # rascore_mat: N x haplotypee
load("T1D_haplotype_v1.RData")
