...
length="13"; pos="6"
 ...
 #read info
 #info <- read.table("assoc/HIP/lmm/v3_length_cov_rm_gl/lead_info.txt",header=T,stringsAsFactors=F)
 #info <- info[ info$length==length & info$pos==pos, ]
 #output file and directory
 #ofile <- paste0("assoc/HIP/lmm/v3_length_jfixed_cov_rm_gl/L", length, ".", pos, ".lmm.v3_length.txt.gz")

 #phenotype
pdata <- read.table(paste0("../../data/tcr_phenotype/L", length, "_P", pos, "_rm_gl/jgene_cond_single_aa_usage_ratio.txt.gz"),
   header=T, stringsAsFactors=F)
pass <- read.table( paste0("../../data/tcr_phenotype/L", length, "_P", pos, "_rm_gl/single_aa_usage_ratio.pass"),
...
res <- data.frame()
 ...
 #for( i in 1:nrow(info) ){
   ...
   ptarget = "E"
   gtarget = "AA_DRB1_13_32552130_HRY"
   HLA = "DRB1"
   
   #genotype (NEW GENOTYPE)
   gdata <- read.table(paste0("../../data/genotype/HLA_", HLA, ".imgt.aa.postqc.matrix"), header=T, stringsAsFactors=F)
   ...
   if( var(M$rate) >0 & var(M$dose) > 0 & length(unique(M$Vgene)) > 1 & length(unique(M$Sample)) > 1 ){
      ...
      dump <- data.frame( pname=ptarget, gname=gtarget,  HLA,
         beta1, se1, lrpval1,
         beta2, se2, lrpval2
         )
